...
import { FilterOption, dropDownOptions } from "@/types/component-types/filter-dropdown-type";
...
export const filteredCampaignsSelector = selector({
  ...
  get: ({ get }) => {
    ...
    switch (selectedFilter.label) {
      case "최신순":
        filtered.sort((a: any, b: any) => new Date(b.startAt).getTime() - new Date(a.startAt).getTime());
        break;
      case "신청자 많은순":
      ...
      case "신청자 낮은순":
        ...
        break;
      case "마감 임박순":
        filtered.sort((a: any, b: any) => new Date(a.endAt).getTime() - new Date(b.endAt).getTime());
        break;
      case "포인트 높은순":
